...
//            	self.docH = docH = canW / wsize * zoomIndex * zoom;
    ...
    Correct.prototype.reverseRecall = function () {
        var self = this
        self.executionArray.pop();
        self.imgs = self.executionArray.length > 0 ? self.executionArray[self.executionArray.length + 1]['img'] : self.img;
        self.drawImage();
    ...
    Correct.prototype.mousedown = function (e, that) {
	    ...
	    if (self.isText) {
	    	...
	    	var h = "<div id='tempTextBox' style='position: absolute; top: "+y+"px; left: "+x+"px;'>"+
	    				"<div style='border: solid 1px #ff5050; width: 200px;'><input style='text-shadow: 1px 0 1px #ffffff, 0 1px 1px #ffffff, \n" +
                "    0 -1px 1px #f7f7f7, -1px 0 1px #f7f7f7;background: rgba(255,255,255,.2); font-size: 14px; color: #ff5050; border: none; outline: none; padding: 5px;width: 100%; height: 100%;' type='text' id='tempTextVal'/></div>"+
	    			"</div>";
...
//		var gcbr = document.querySelector('#'+correct.id).getBoundingClientRect();
    ...
    $(document).on('click', '#reverseRecall', function(e){
        if (!correct2) {
            return;
        }
        correct2.reverseRecall();
    });
	// 第二步图片编辑画布绑定鼠标按下事件
